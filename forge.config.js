...
module.exports = {
  hooks: {
    // ビルド前にdata/schema_v2.sqlをframework/data/に同期し、バージョン情報を埋め込む
    generateAssets: async () => {
      // schema_v2.sql を framework/data/ に同期
      ...
      // package.json のバージョンをビルド成果物（src/main/version.ts）に埋め込む
      const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, 'package.json'), 'utf8'));
      const versionTsPath = path.join(__dirname, 'src', 'main', 'version.ts');
      const versionContent = `// Auto-generated by forge.config.js generateAssets hook
// Do not edit manually.
export const APP_VERSION = '${pkg.version}';
export const BUILD_DATE = '${new Date().toISOString().split('T')[0]}';
`;
      fs.writeFileSync(versionTsPath, versionContent, 'utf8');
      console.log(`[forge] Embedded version ${pkg.version} into src/main/version.ts`);
    },
  },
  packagerConfig: {
    asar: {
