...
# DRAFT ORDER一覧を取得（バックログ相当）
python backend/order/list.py $PROJECT_NAME --draft --json
...
```
DBモードでの状態判定:
1. アクティブORDER数が0 → COMPLETED または INITIAL または DRAFT_ONLY
   - 全ORDER完了済み → COMPLETED
   - ORDERなし → INITIAL
   - DRAFT ORDERのみ存在 → DRAFT_ONLY（DRAFT一覧を表示し昇格を提案）
...
# 2. DRAFT ORDER取得（バックログ相当）
python backend/order/list.py AI_PM_PJ --draft --json
