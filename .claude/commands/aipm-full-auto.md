...
```bash
# DB利用可能判定（backend/スクリプト経由）
python backend/project/list.py --json > /dev/null 2>&1 && echo "DBモード" || echo "従来方式"
```

### 1.2 プロジェクト存在確認

```bash
# DBモード（backend/スクリプト経由）
...
2. **ORDER読み込み**
   - `backend/order/list.py` 経由でORDER情報を取得（直接ファイルパスアクセス禁止）
...
for task in parallel_tasks:
    # ステータス更新
    python backend/task/update.py → IN_PROGRESS
...
while running_tasks and elapsed < MAX_POLL_TIME:
    for task in running_tasks:
        ...
        task_status = python backend/task/get.py → status
        if task_status == "DONE":
            → 完了処理へ（4.7.2）
...
def execute_review_flow(task_id):
    ...
    status = python backend/task/get.py → status
    if status != "DONE":
        # IN_PROGRESSのままならDONEに更新
        python backend/task/update.py → DONE
