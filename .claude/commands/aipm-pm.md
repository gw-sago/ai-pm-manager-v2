...
> ## ⚠️ 推奨: DB操作には必ず `--script` モードを使用してください
>
> `/aipm-pm` でDB操作（ORDER作成・タスク発行・状態更新）を行う際は、**`--script` オプションを付けて実行することを強く推奨**します。
>
> **理由**:
> - `--script` を使用しない場合、Claude が直接 `python backend/...` を呼び出す際に誤ったパスや引数でDB操作を行うリスクがあります
> - `--script` モードは `backend/pm/process_order.py` が一括処理するため、フロー漏れ・DB不整合が発生しません
> - トークン消費も50〜70%削減できます
>
> **推奨コマンド例**:
> ```bash
> /aipm-pm PROJECT_NAME ORDER_NUMBER --script
> ```
>
> やむを得ず `--script` なしで実行する場合は、各ステップで `python backend/` スクリプトを正しいパスで呼び出してください。直接SQLiteを操作したり、旧パス（`aipm-db/` 等）を使用することは**禁止**です。
...
## 自然言語の意図判定（最優先 - 引数解析より先に評価）

ユーザーの発言（`$ARGUMENTS` やコマンド実行前の会話）に以下のキーワード・意図が含まれる場合、**明示的な `--draft` フラグがなくても自動的にモード0（DRAFT ORDER作成）として処理する**。

### DRAFTモードに自動ルーティングするキーワード:
- 「壁打ち」「ブレスト」「アイデア出し」「相談」
- 「バックログに追加」「バックログ追加」「バックログに登録」
- 「下書き」「ドラフト」「DRAFT」
- 「登録だけ」「追加だけ」「メモしておいて」
- 「あとで実行」「実行はしないで」「まだ実行しない」「実行しなくていい」

### 判定ロジック:
```
IF ユーザーの発言に上記キーワードが含まれる THEN
    1. ユーザーと壁打ち（議論・ブレスト）を行う
    2. 議論の結果をDRAFT ORDERとして登録する（Mode 0の手順）
    3. **PM処理（要件定義・タスク発行）は絶対に実行しない**
    4. 登録後、次のアクション（PLANNING昇格 or 追加の壁打ち）を提示する
    → 処理終了
END IF
```

### 壁打ちフロー:
1. ユーザーの課題・アイデアについて対話的に議論する
2. 議論を整理してタイトルと概要（description）をまとめる
3. ユーザーに確認してからDRAFT ORDERとして登録する:
   ```bash
   python backend/order/create.py $PROJECT_NAME --title "タイトル" --description "概要" --status DRAFT --json
   ```
4. 登録後、次のアクションを提示:
   - PLANNINGに昇格してPM処理開始: `/aipm-pm PROJECT_NAME ORDER_NUMBER`
   - 完全自動実行: `/aipm-full-auto PROJECT_NAME ORDER_NUMBER`

**重要**: 壁打ち・バックログ追加の意図がある場合、**絶対にフルPM処理（Mode 1以降）に進まないこと**。DRAFT ORDER登録のみで処理を完了する。
