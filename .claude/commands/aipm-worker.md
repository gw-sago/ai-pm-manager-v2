...
> **⚠️ 注意**: Worker識別子の割当はIN_PROGRESSタスク一覧（Section 1.7.1）から判定してください。
> 旧形式 `python -m aipm-db.worker.assign` は廃止済みです。`backend/task/list.py` を使用してください。

**次のWorker識別子を取得**:

```bash
# IN_PROGRESSタスクの担当Workerを確認して、未使用の識別子を決定
python backend/task/list.py $PROJECT_NAME --status IN_PROGRESS --json
```
...
# 出力: IN_PROGRESSタスク一覧 → assigneeフィールドから使用中Workerを確認
# Worker Aが使用中 → Worker Bを割当
```

**処理結果**:
- DBからIN_PROGRESSタスクの担当Workerを取得
- 使用されていない最小の識別子を割当（Worker A → B → C → ...）
- 全Worker使用中の場合はエラー

**Worker状況確認**:
```bash
python backend/task/list.py AI_PM_PJ --status IN_PROGRESS --json
# 出力のassigneeフィールドで使用中Worker一覧を確認
# 例: Worker AとWorker Bが使用中 → 次はWorker Cを割当
```
...
> **⚠️ 注意**: 旧形式 `python PROJECTS/$PROJECT_NAME/DEV/aipm-db/task/update.py` および
> `python -m aipm-db.task.update` は廃止済みです。必ず `backend/task/update.py` を使用してください。
...
> **⚠️ 注意**: 旧形式 `cd PROJECTS/$PROJECT_NAME/DEV && python -m aipm-db.session.checkpoint` は廃止済みです。
> `backend/session/checkpoint.py` を使用してください。スクリプトが存在しない場合はフォールバック（3.5.6参照）。

**スクリプト呼び出し**:

```bash
python backend/session/checkpoint.py $PROJECT_NAME --step "ステップ名" --next "次のアクション" --memo "メモ"
```

**実行例**:
```bash
python backend/session/checkpoint.py AI_PM_PJ --step "aipm-worker.md編集完了" --next "テスト実施" --memo "Step 1.5, 3.5更新"
```
...
**ファイル作成完了時（CP-T01）**:
```bash
python backend/session/checkpoint.py AI_PM_PJ --step "checkpoint_design.md作成完了" --next "次ステップ実行" --memo "設計書v1.0"
```

**重要判断完了時（CP-T04）**:
```bash
python backend/session/checkpoint.py AI_PM_PJ --step "記録方式決定" --next "設計書作成" --memo "履歴保持方式を採用"
```

**マイルストーン到達時（CP-T05）**:
```bash
python backend/session/checkpoint.py AI_PM_PJ --step "完了条件1達成" --next "次の条件" --memo "3種類のトリガー定義完了"
```
...
【Worker識別子取得】
$ python backend/task/list.py AI_PM_PJ --status IN_PROGRESS --json
# assigneeフィールド確認 → 未使用のWorker識別子を決定
Worker A
...
$ python backend/task/update.py AI_PM_PJ TASK_192 --status IN_PROGRESS --assignee "Worker A" --role Worker
...
DB更新が完了しました。
```
→ タスク開始時、Worker識別子確認→タスクステータス更新の順でスクリプトを実行
...
【チェックポイント記録】
$ python backend/session/checkpoint.py AI_PM_PJ --step "aipm-worker.md編集完了" --next "テスト実施" --memo "Step 1.5, 3.5, 1.7更新"
...
> **⚠️ 重要**: 全スクリプトは `backend/` 配下にあります。
> 旧形式 `cd PROJECTS/$PROJECT_NAME/DEV && python -m aipm-db.*` は廃止済みです。
> 直接 `python backend/xxx/yyy.py` 形式で呼び出してください。

### スクリプト一覧

| スクリプト | 用途 | 呼び出しタイミング |
|-----------|------|-------------------|
| `backend/task/update.py` | タスクステータス更新 | タスク開始時、完了時、中断時 |
| `backend/task/get.py` | タスク情報取得 | タスク情報確認時 |
| `backend/task/list.py` | タスク一覧取得 | 依存関係確認時 |
| `backend/session/checkpoint.py` | チェックポイント記録 | 作業完了時（Step 3.5） |

### 基本的な使い方

```bash
# タスクステータス更新
python backend/task/update.py $PROJECT_NAME TASK_$TASK_ID --status IN_PROGRESS --role Worker

# タスク情報取得
python backend/task/get.py $PROJECT_NAME TASK_$TASK_ID --json

# 差し戻しタスク確認（REWORKステータス）
python backend/task/list.py $PROJECT_NAME --status REWORK

# チェックポイント記録
python backend/session/checkpoint.py $PROJECT_NAME --step "ステップ名" --next "次のアクション" --memo "メモ"

# Worker識別子確認（IN_PROGRESSタスクのassigneeから判定）
python backend/task/list.py $PROJECT_NAME --status IN_PROGRESS --json
```
