...
class WorkerExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        # 開発環境パス（DBから取得、NULLの場合はNone）
        self.dev_workspace_path: Optional[str] = self._get_dev_workspace_path()
    ...
    def _get_dev_workspace_path(self) -> Optional[str]:
        """DBからプロジェクトのdev_workspace_pathを取得"""
        try:
            conn = get_connection()
            try:
                # カラム存在チェック
                cols = [row["name"] for row in fetch_all(conn, "PRAGMA table_info(projects)")]
                if "dev_workspace_path" not in cols:
                    return None
                row = fetch_one(
                    conn,
                ...
                if row and row["dev_workspace_path"]:
                    return row["dev_workspace_path"]
                return None
            finally:
                conn.close()
        except Exception as e:
            logger.debug(f"dev_workspace_path取得失敗: {e}")
            return None
    ...
    def _step_execute_task(self) -> None:
        ...
        # 成果物出力先ディレクトリを事前作成（RESULT/ORDER_XXX/06_ARTIFACTS/）
        if self.order_id:
            artifact_dir = self.project_dir / "RESULT" / self.order_id / "06_ARTIFACTS"
            artifact_dir.mkdir(parents=True, exist_ok=True)
            self._log_step("execute_task", "info", f"06_ARTIFACTS dir ensured: {artifact_dir}")
...
"""
        ...
        # 成果物出力先（RESULT/ORDER_XXX/06_ARTIFACTS/）
        artifact_dir_str = ""
        if self.order_id:
            artifact_dir = self.project_dir / "RESULT" / self.order_id / "06_ARTIFACTS"
            artifact_dir_str = str(artifact_dir)
        else:
            artifact_dir_str = str(self.project_dir / "RESULT" / "<ORDER_ID>" / "06_ARTIFACTS")
...
| 成果物出力先 (06_ARTIFACTS) | `{artifact_dir_str}` |

**成果物の配置ルール**: タスクで作成・更新したファイルのパスを `artifacts` フィールドに記録してください。
成果物ファイルを専用ディレクトリにコピーする必要はありません。絶対パスをそのまま列挙してください。

**理由**: Squirrelインストーラーの更新でAppData\\Localが上書きされるため、永続データは必ずAppData\\Roaming配下に配置する必要があります。

"""

        # 開発環境パスセクション（dev_workspace_pathが設定されている場合のみ）
        dev_workspace_section = ""
        if self.dev_workspace_path:
            dev_workspace_section = f"""
## ⚠️ 開発環境パスルール（ソースコード変更は開発環境で！）

このプロジェクトには開発環境パス（ソースリポジトリ）が設定されています。

**開発環境パス**: `{self.dev_workspace_path}`

### 必須ルール
1. **ソースコード・スクリプトの変更**は必ず開発環境パスで行ってください
   - `{self.dev_workspace_path}/backend/` 配下のPythonスクリプト
   - `{self.dev_workspace_path}/src/` 配下のTypeScript/React コード
   - `{self.dev_workspace_path}/data/` 配下のスキーマファイル
   - その他ビルド成果物に含まれるファイル全般

2. **Roaming側（`{self.project_dir}` 配下）のbackend/スクリプトを直接編集してはならない**
   - Roaming側を直接変更するとソースと乖離し、次回ビルドで上書き・残存してデグレの原因になる

3. **使い分け**
   | 操作 | パス |
   |------|------|
   | ソースコード変更 | `{self.dev_workspace_path}` |
   | PROJECTS/配下のデータ読み書き | `{self.project_dir}` (Roaming) |
   | 成果物の記録 | `{self.project_dir}` (Roaming) |
   | DB操作（読み取り） | Roaming側DB |
   | マイグレーション開発・テスト | `{self.dev_workspace_path}/data/test_aipm.db` |
...
{rework_section}{rework_history_section}{failure_context_section}{migration_section}{worker_env_section}{roaming_path_section}{dev_workspace_section}{test_file_rules_section}{known_bugs_section}
## タスク定義
{task_content}
